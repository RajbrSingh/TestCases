...
public class Logout {
	...
	public static void logout(WebDriver driver, Logger logger, String loginType, String country, String language,
			String baseURL) {
		try {
			...
			if (loginType.equalsIgnoreCase("Drupal")) {
				...
				if (titleText.equalsIgnoreCase("Global Intranet") || titleText.equalsIgnoreCase("Intranet global")) {
					...
					if (country.equalsIgnoreCase("CA") || country.equalsIgnoreCase("NA")) {
						...
						try {
							assertEquals(driver.findElement(By.cssSelector("input#okta-signin-username")).getText()
									.toString(), "");
